        ...
        return new Promise((resolve) => {
            ...
            if (verbose) {
                if (onProgress) {
                    ...
                    (0, ui_1.printStatus)('exec', `launching danser: ${path.basename(this.danserBin).toLowerCase()} ${args.join(' ').toLowerCase()}`);
                }
                ...
                child.on('error', (err) => {
                    (0, ui_1.printStatus)('render', `failed to start danser binary: ${err.message.toLowerCase()}`, 'error');
            ...
            const handleChunk = (chunk) => {
                ...
                for (const rawLine of lines) {
                    ...
                    if (progressMatch) {
                        ...
                        const detailParts = [];
                        if (fps)
                            detailParts.push(`fps: ${fps}`);
                        if (eta)
                            detailParts.push(`eta: ${eta}`);
